...
class PlayState extends MusicBeatState
{
	...
	public var dadfoucused:Bool = false;
	...
	var cdbase:AttachedSprite;
	var cdtext:AttachedSprite;
...
//		boyfriendCameraOffset = stageData.camera_boyfriend;
		...
		switch (curStage)
		{
			...
			case 'jonicbg': 
					var bg:BGSprite = new BGSprite('jonic/bg', -600, -667);
					bg.scrollFactor.set(0.5, 1);
					bg.setGraphicSize(Std.int(bg.width * 1.3));
					bg.animation.addByPrefix('idle', 'bganim', 24, true);
					bg.dance(true);
					bg.antialiasing = false;
					bg.updateHitbox();
					add(bg);
					
					var fg:BGSprite = new BGSprite('jonic/fg', -1755, -100, 0.9, 0.9);
					fg.antialiasing = false;
					add(fg);


			case 'bridge':
				var bg:BGSprite = new BGSprite('newbg3', -685, -390, 0.9, 0.9);
				add(bg);
		}
		...
		FlxG.camera.follow(camFollowPos, LOCKON, 3);
		...
		cdbase = new AttachedSprite('soniccdreal/base');
		cdbase.y -= 500;
		cdbase.scrollFactor.set();
		//startCircle.screenCenter(Y);
		add(cdbase);
		cdtext = new AttachedSprite('soniccdreal/' + Paths.formatToSongPath(curSong));
		cdtext.x += 500;
		cdtext.scrollFactor.set();
		add(cdtext);
		...
		noteTxt = new FlxText(0, 355, 200, "", 18);
		noteTxt.setFormat(Paths.font("vcr.ttf"), 18, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		noteTxt.scrollFactor.set();
		noteTxt.screenCenter(Y);
		noteTxt.borderSize = 1.25;
		noteTxt.text = 'Sicks: ${sicks}\nGoods: ${goods}\nBads: ${bads}\nShits: ${shits}';
		noteTxt.visible = !ClientPrefs.hideHud;
		add(noteTxt);
		
		watermark = new FlxText(0, 690, 512, "", 16);
		...
		watermark.text = songName + '  -  ' + storyDifficultyText;
		...
		doof.cameras = [camHUD];
		cdbase.cameras = [camHUD];
		cdtext.cameras = [camHUD];
	...
	function startSong():Void
	{
		...
		new FlxTimer().start(0.6, function(tmr:FlxTimer)
			{
				FlxTween.tween(cdbase, {y: 0}, 0.5);
				FlxTween.tween(cdtext, {x: 0}, 0.5);
			});

			new FlxTimer().start(1.5, function(tmr:FlxTimer)
			{
				FlxTween.tween(cdbase, {y: -999}, 0.4,
				{						
					ease: FlxEase.cubeInOut,
					onComplete: function(twn:FlxTween) {
					cdbase.destroy();
				...
				FlxTween.tween(cdtext, {x: 999}, 0.4,
				{						
					ease: FlxEase.cubeInOut,
					onComplete: function(twn:FlxTween) {
					cdtext.destroy();
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Set Camera Zoom':
					var camZoom:Float = Std.parseFloat(value1);
					if(Math.isNaN(camZoom)) camZoom = 0.88;
...
/*	if (dadfoucused)
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			if(dadfoucused && Paths.formatToSongPath(SONG.song) != 'fuckyou'){
